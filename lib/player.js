export class Player{
    ...
    async tick(){
        ...
        this.game.debug.addEntry(`VelX : ${Math.round(this.velX)}, VelY : ${Math.round(this.velY)}`)
        this.game.debug.addEntry(`Lives : ${Math.round(this.lives)}`)
        this.game.debug.addEntry(`Colidable? : ${this.colide}`)
        if(this.y>140){
            if(this.lives==0){
