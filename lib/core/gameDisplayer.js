...
export class Display{
    ...
    toasts = []
    colors=[
        ["#33363f","#a7c7d8","#ffffff", ["#afbfaf", "#c0b090", "#bfafbf", "#bfcfaf", "#dfafaf", "#ddd"],"#00000050","#33363f","#fff5","#dfdf8d"],
        ["#000000","#90b0c0","#ffffff",["#afbfaf", "#c0b090", "#bfafbf", "#bfcfaf", "#dfafaf", "#ddd"],"#00000050","#000000","#fff5","yellow"]]
    constructor(game){
        this.game=game
    }
    tick(){
    ...
    drawGame(){
        ...
        actx.strokeText(`$${this.game.player.money}`, 5, 101)
        actx.fillStyle = this.colors[this.game.menu.options[0].index][2]
        actx.fillText(`${this.game.player.score}`, 5, 17)
        // actx.fillStyle = this.colors[this.game.menu.options[0].index][7]
        actx.fillText(`$${this.game.player.money}`, 5, 101)
    }
    drawGameAnim(anim){
        actx.fillStyle = this.colors[this.game.menu.options[0].index][0]
        for(let i = 0; i<5; i++){
    ...
    drawSmooth(anim){
        ...
        for(let i = 0; i < this.toasts.length; i++){
            ...
            ctx.fillStyle = this.colors[this.game.menu.options[0].index][3][0]
        ...
        ctx.save()
        ctx.translate(this.game.displayinfo.aspectRatio[0],this.game.displayinfo.aspectRatio[1]/2)
        ctx.beginPath()
        ctx.arc(900-anim[0],0,650+anim[2]*3,0,100,false)
        ctx.fillStyle = `${this.colors[this.game.menu.options[0].index][1]}99`
        ctx.fill()
        ctx.fillStyle = this.colors[this.game.menu.options[0].index][4]
        ctx.fill()
        ...
        this.game.shop.draw(ctx,anim[2])
        ctx.restore()

        this.game.debug.draw(ctx)
