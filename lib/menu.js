...
export class Menu{
    game
    offset=0
    options = [new Setting("Theme","options","Classic",["Classic","Original"]),new Setting("Pixel Perfect?","boolean",true,[false,true],1),new Setting("option 3"),new Setting("Clear Save?","boolean",false,[false,true],0)]
    selected = 0
    selectedSmooth=0
    settingState=0
    ...
    drawSettingThing(i,ctx){
            ...
            for(let z = 0; z < this.options[i].options.length;z++){
                ...
                ctx.fillStyle = this.game.display.colors[this.game.menu.options[0].index][3]
